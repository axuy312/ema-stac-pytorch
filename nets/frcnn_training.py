...
class FasterRCNNTrainer(nn.Module):
    def __init__(self, model_train, optimizer, threshold=0.3):
    ...
    def forward(self, imgs, bboxes, labels, scale, sup=True):
        n           = imgs.shape[0]
        ...
        rpn_loc_loss_all, rpn_cls_loss_all, roi_loc_loss_all, roi_cls_loss_all  = torch.Tensor([0]), torch.Tensor([0]), torch.Tensor([0]), torch.Tensor([0])
        rpn_loc_loss_all = rpn_loc_loss_all.to(imgs.device)
        rpn_cls_loss_all = rpn_cls_loss_all.to(imgs.device)
        roi_loc_loss_all = roi_loc_loss_all.to(imgs.device)
        roi_cls_loss_all = roi_cls_loss_all.to(imgs.device)
        ...
        for i in range(n):
            ...
            if sup:
                ...
                Wb = F.softmax(rpn_score,dim=1)[range(gt_rpn_label.shape[0]), gt_rpn_label.view(-1)]>self.threshold
                if rpn_score[Wb].nelement() != 0:
                    #rpn_loc_loss = self._fast_rcnn_loc_loss(rpn_loc[Wb], gt_rpn_loc[Wb], gt_rpn_label[Wb], self.rpn_sigma)
        ...
        for i in range(n):
            ...
            if sup:
                ...
                Wb = F.softmax(roi_score,dim=1)[range(gt_roi_label.shape[0]), gt_roi_label.view(-1)]>self.threshold
                if roi_score[Wb].nelement() != 0:
                    roi_cls_loss = nn.CrossEntropyLoss()(roi_score[Wb], gt_roi_label[Wb])
                    roi_loc_loss = self._fast_rcnn_loc_loss(roi_loc[Wb], gt_roi_loc[Wb], gt_roi_label.data[Wb], self.roi_sigma)
                    roi_loc_loss_all += roi_loc_loss
                    roi_cls_loss_all += roi_cls_loss
