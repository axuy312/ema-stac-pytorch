...
def fit_one_epoch(model, train_util, loss_history, eval_callback, optimizer, epoch, epoch_step, epoch_step_val, gen_sup, gen_unsup, gen_val, Epoch, cuda, fp16, scaler, save_period, save_dir):
    total_loss = 0
    rpn_loc_loss = 0
    rpn_cls_loss = 0
    roi_loc_loss = 0
    ...
    print('Start Supervise Train')
    with tqdm(total=len(gen_sup),desc=f'Epoch {epoch + 1}/{Epoch}',postfix=dict,mininterval=0.3) as pbar:
        for iteration, batch in enumerate(gen_sup):
            ...
            rpn_loc, rpn_cls, roi_loc, roi_cls, total = train_util.train_step(images, boxes, labels, 1, fp16, scaler, True)
        ...
        with tqdm(total=len(gen_unsup),desc=f'Epoch {epoch + 1}/{Epoch}',postfix=dict,mininterval=0.3) as pbar:
            ...
            for iteration, batch in enumerate(gen_unsup):
                ...
                pbar.set_postfix(**{'total_loss'    : total_loss / count,
                                    ...
                                    'roi_cls'       : roi_cls_loss / count,
                                    'lr'            : get_lr(optimizer)})
                pbar.update(1)
        print('Finish Unsupervise Train')
    
    
    print('Start Validation')
    with tqdm(total=len(gen_val), desc=f'Epoch {epoch + 1}/{Epoch}',postfix=dict,mininterval=0.3) as pbar:
