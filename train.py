...
if __name__ == "__main__":
    ...
    # Unlabeled max #
    #---------------------------#
    unlabeled_data_max_num = -1
    
    #----------------------------------------------------#
    # 獲取圖片路徑和標籤
    # --------------------------------------------------#
    train_annotation_path_sup   = 'train.txt'
    train_img_path_unsup   = 'dataset/PCB/JPEGImages_unlabeled'
    ...
    if model_path != '':
        ...
        print("\n\033[1;33;44m溫馨提示，head部分沒有載入是正常現象，Backbone部分沒有載入是錯誤的。\033[0m")
    ...
    # --------------------------------------------------#
    # EMA更新的Teacher model(device要是CPU)
    # --------------------------------------------------#
    if train_mode == "Student":
        ema_teacher = EMA(model, ema_decay)
    else:
        ema_teacher = None
    ...
    #----------------------------------------------------------------------------#
    # 讀取數據集對應的txt (Labeled data for training)
    #----------------------------------------------------------------------------#
    with open(train_annotation_path_sup, encoding='utf-8') as f:
        train_lines_sup = f.readlines()
    
    #----------------------------------------------------------------------------#
    # 模擬訓練集不足(Labeled data for training)
    #----------------------------------------------------------------------------#
    tmp_train_lines_sup = train_lines_sup
    train_lines_sup = []
    for i, l in enumerate(tmp_train_lines_sup):
        if i % 2 == 0:
            train_lines_sup.append(l)
    
    #----------------------------------------------------------------------------#
    # 讀取數據集對應的txt (Unlabeled data for training)
    #----------------------------------------------------------------------------#
    
    train_lines_unsup = []
    if train_mode == "Student":
        for fn in os.listdir(train_img_path_unsup):
            fn_info = fn.split(".")
            if len(fn_info) > 0 and fn_info[-1] == "jpg":
                train_lines_unsup.append(os.path.join(train_img_path_unsup, fn))
    '''
    with open("unsup.txt", encoding='utf-8') as f:
        train_lines_unsup = f.readlines()
    '''
    #----------------------------------------------------------------------------#
    # 讀取數據集對應的txt (Unlabeled data for validation)
    #----------------------------------------------------------------------------#
    ...
    show_config(
        ...
        Init_Epoch = Init_Epoch, Freeze_Epoch = Freeze_Epoch, UnFreeze_Epoch = UnFreeze_Epoch, Freeze_Train = Freeze_Train, batch_size = batch_size, mini_batch_size= mini_batch_size, \
        Init_lr = Init_lr, Min_lr = Min_lr, optimizer_type = optimizer_type, momentum = momentum, lr_decay_type = lr_decay_type, \
        save_period = save_period, save_dir = save_dir, eval_period = eval_period, num_workers = num_workers, \
        num_train = num_train_sup, num_train_unsup = num_train_unsup, num_val = num_val, \
        ema_decay = ema_decay, unlabeled_data_max_num = (num_train_unsup if unlabeled_data_max_num == -1 else unlabeled_data_max_num)
    )
    ...
    if True:
        ...
        if train_mode == "Teacher":
            ...
            train_dataset_unsup = FRCNNDataset(train_lines_unsup, input_shape, train = True, max_num = unlabeled_data_max_num)
        ...
        eval_callback   = EvalCallback(model_train, input_shape, class_names, num_classes, val_lines, log_dir, Cuda, \
                                        eval_flag=eval_flag, period=eval_period, teacher=(ema_teacher.model if train_mode == "Student" and ema_decay < 1 else None))
        
        #----------------------#
        # 紀錄初始mAP(epoch=0)
        #----------------------#
        eval_callback.on_epoch_end(epoch=Init_Epoch)
        ...
        for epoch in range(Init_Epoch, UnFreeze_Epoch):
            ...
            if epoch >= Freeze_Epoch and not UnFreeze_flag and Freeze_Train:
                ...
                # 解凍一定部分訓練
                # ------------------------------------#
                print(f"UnFreeze model.(Epoch {epoch + 1})")
                for param in model.extractor.parameters():
            ...
            if train_mode == "Student" and epoch == 0:
                train_dataset_unsup.resample_data(max_num = unlabeled_data_max_num)
                train_dataset_unsup.updateGT(ema_teacher.model)
                gen_unsup = DataLoader(train_dataset_unsup, shuffle = True, batch_size = batch_size, num_workers = num_workers, pin_memory=True,
                                        drop_last=True, collate_fn=frcnn_dataset_collate, 
                                        worker_init_fn=partial(worker_init_fn, rank=0, seed=seed))
            
            fit_one_epoch(model, train_util, loss_history, eval_callback, optimizer, epoch, gen_sup, gen_unsup, gen_val, UnFreeze_Epoch, Cuda, fp16, scaler, save_period, save_dir, ema_teacher=ema_teacher, mini_batch_size=mini_batch_size)
